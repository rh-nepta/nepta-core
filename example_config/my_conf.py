...
host_1.interfaces.eth.eth_0 = EthernetInterface(
    'eth_0', '1F:5A:22:2B:21:80', IPv4Configuration(
        [IPv4Interface('172.16.0.1/24'), IPv4Interface('172.16.0.2/24')], gw, dns
    )
)

# assign test IP address from network configuration generator
host_1.interfaces.eth.eth_1 = EthernetInterface(
    'eth_1', 'A2:A0:8B:4B:B5:C7', net4.new_config(), net6.new_config())

# Create interfaces configuration for second host
# statically assign IP addresses to non test interface
host_2.interfaces.eth.eth_0 = EthernetInterface(
    'eth_0', '1F:5A:22:2B:21:81', IPv4Configuration(
        [IPv4Interface('172.16.0.11/24'), IPv4Interface('172.16.0.12/24')], gw, dns
    )
)

# assign test IP address from network configuration generator
host_2.interfaces.eth.eth_1 = EthernetInterface(
    'eth_1', 'A2:A0:8B:4B:B5:C8', net4.new_config(), net6.new_config())

paths = [
    P(host_1.interfaces.eth.eth_1.v4_conf[0], host_2.interfaces.eth.eth_1.v4_conf[0],
      [tag.SoftwareInventoryTag('IPv4')]),
    P(host_1.interfaces.eth.eth_1.v6_conf[0], host_2.interfaces.eth.eth_1.v6_conf[0],
