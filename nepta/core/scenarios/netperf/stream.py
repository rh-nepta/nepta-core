...
class NetperfTCPStream(SingleStreamGeneric):
    ...
    def init_test(self, path, size):
        netperf_test = NetperStreamfTest(src_ip=path.mine_ip, dst_ip=path.their_ip, length=self.test_length, local_send=size,
                                         test='TCP_STREAM', )
        if self.cpu_pinning:
...
class NetperfTCPDuplexStream(DuplexStreamGeneric):
    ...
    def init_all_tests(self, path, size):
        netperf_test = NetperStreamfTest(src_ip=path.mine_ip, dst_ip=path.their_ip, length=self.test_length,
                                         local_send=size, test='TCP_STREAM')
        maerts_test = NetperStreamfTest(src_ip=path.mine_ip, dst_ip=path.their_ip, length=self.test_length,
                                        local_send=size, test='TCP_MAERTS')
        if self.cpu_pinning:
    ...
    def parse_all_results(self, tests):
        ...
        result_dict['avg_local_cpu'] = round_f(float(stream_test_result['loc_util']) + float(maerts_test_result['loc_util']))
        result_dict['avg_remote_cpu'] = round_f(float(stream_test_result['rem_util']) + float(maerts_test_result['rem_util']))
...
class NetperfTCPMultiStream(MultiStreamsGeneric):
    ...
    def init_all_tests(self, path, size):
        ...
        for pinning in self.cpu_pinning:
            new_test = NetperStreamfTest(src_ip=path.mine_ip, dst_ip=path.their_ip, length=self.test_length,
                                         local_send=size, test='TCP_STREAM')
